<?php
	...
	class hw_errors{
		...
		function __construct( $showBacktrace = false ){
			if( !hiweb()->context()->is_frontend_page() && !hiweb()->context()->is_backend_page() )
				return;
			hiweb()->css( hiweb()->dir_css . '/errors.css' );
		...
		public function errorHandler( $errno, $errstr, $errfile, $errline ){
			if( preg_match( '/(wp-admin|wp-include)/g', $errfile ) > 0 )
				return;
			...
			$r .= "</p>";
			if( $errno == E_ERROR )
				print $r; else $this->putToFooter( $r );
		}
		...
		public function echo_footerErrorsHtml(){
			echo implode( '', $this->footerErrorsHtml );
		}
