	...
	class hiweb_console {
		...
		public function info( $info, $debugMod = false ) {
			...
			$this->_mess[]        = array(
				...
				'file'      => hiweb()->backtrace()->file_locate( $callFromConstruct ? 3 : 2 ),
		...
		public function warn( $info, $debugMod = false ) {
			...
			$this->_mess[]        = array(
				...
				'debug'     => $debugMod,
				'microtime' => microtime( 1 ),
				'file'      => hiweb()->backtrace()->file_locate( 2 ),
				'function'  => hiweb()->backtrace()->function_trace( 2 )
			);
		}
		...
		public function echo_footer() {
			...
			foreach ( $this->_mess as $info ) {
				$this->{'echo_' . $info['type']}( $info['debug'] ? ( is_array( $info['data'] ) ? array_merge( array( '► ' . $info['function'], '► ' . $info['file'] ), $info['data'] ) : $info['function'] . '() : ' . $info['data'] . chr( 13 ) . chr( 10 ) . '► ' . $info['file'] ) : $info['data'] );
			}
			echo '</script>';
		}
