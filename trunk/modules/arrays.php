	...
	class hiweb_arrays{
		...
		public function getVal( $haystack = array(), $keyMix = '', $def = null ){
			...
			if( is_array( $keyMix ) && count( $keyMix ) > 1 ){
				...
				return $this->getVal( $this->getVal( $haystack, $key, $def ), $keyMix, $def );
		...
		public function explodeTrim( $delimiter, $haystack, $returnEmptyParts = true, $returnEmptyArray = true ){
			...
			foreach( explode( $delimiter, $haystack ) as $part ){
				if( $returnEmptyParts || trim( $part ) != '' ) $r[] = $part;
			}
		...
		public function count( $haystack, $keyMix = null ){
			if( !is_null( $keyMix ) ){
				$haystack = $this->getVal( $haystack, $keyMix );
			}
		...
		public function group_by_value( $array, $groupKey ){
			$R = array();
			if( is_array( $array ) ) foreach( $array as $key => $haystack ){
		...
		public function ungroup( $array, $level = 0 ){
			...
			if( is_array( $array ) ) foreach( $array as $key => $value ){
				if( $level < 1 ){
					if( is_array( $value ) || is_object($value) ) foreach( (array)$value as $subKey => $subValue ) $R[ $subKey ] = $subValue;
					else $R[] = $value;
				} else{
