<?php
	...
	class hiweb_wp{
		...
		public function post( $postOrId ){
			if( $postOrId instanceof WP_Post ) $postId = $postOrId->ID;
			else $postId = $postOrId;
			///
			if( !isset( $this->posts[ $postId ] ) ) $this->posts[ $postId ] = new hiweb_wp_post( $postOrId );
		...
		public function theme( $theme = null ){
			if( !is_string( $theme ) || trim( $theme ) == '' ) $theme = get_option( 'template' );
	...
	class hiweb_wp_post{
		...
		public function meta( $key = null, $use_regex_index = null ){
			if( !$this->meta instanceof hiweb_wp_meta ) $this->meta = new hiweb_wp_meta( $this->object );
		...
		public function taxonomy( $taxonomy ){
			if( !isset( $this->taxonomies[ $taxonomy ] ) ){
				if( $this->taxonomy_exist( $taxonomy ) ) $this->taxonomies[ $taxonomy ] = false;
	...
	class hiweb_wp_meta{
		...
		public function arr( $key_regex_pattern = null ){
			if( is_null( $key_regex_pattern ) ) return $this->meta;
			////
			$R = array();
			foreach( $this->meta as $key => $val ){
				if( preg_match( $key_regex_pattern, $key ) > 0 ) $R[ $key ] = $val;
			}
			return $R;
		}
			...
			if( !array_key_exists( $argsString, $this->terms ) ){
				...
				if( is_array( $terms ) ){
					if( is_string( $returnKeyGoup ) && trim( $returnKeyGoup != '' ) ){
						foreach( $terms as $term ) if( property_exists( $term, $returnKeyGoup ) ) $this->terms[ $argsString ][ $term->{$returnKeyGoup} ][] = $term;
					} else foreach( $terms as $term ){
						$this->terms[ $argsString ][ $term->term_id ] = $term;
					}
				} else $this->terms[ $argsString ] = array();
			}
	...
	class hiweb_wp_theme{
		...
		public function locations(){
			...
			if( isset( $mods[ 'nav_menu_locations' ] ) ) $R = $mods[ 'nav_menu_locations' ];
			return $R;
		}
